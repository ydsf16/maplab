#!/usr/bin/env python3
"""Prepare pose-conditioned DA3 input from an optimized single-session VI-BA."""
...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--slam-output',type=Path,required=True); p.add_argument('--data',type=Path,required=True); p.add_argument('--output',type=Path,required=True); p.add_argument('--slam-stage',choices=('auto','initial','final'),default='auto'); p.add_argument('--min-translation-m',type=float,default=.15); p.add_argument('--min-rotation-deg',type=float,default=5); p.add_argument('--max-interval-s',type=float,default=1.0); p.add_argument('--max-frames',type=int,default=0); p.add_argument('--start-index',type=int,default=0); p.add_argument('--end-index',type=int,default=-1); p.add_argument('--metadata-only',action='store_true'); a=p.parse_args(); a.output.mkdir(parents=True,exist_ok=True)
 stages={'initial':'04_initial_vi_ba_intrinsics','final':'08_visual_inertial_ba_loops_preview'}
 if a.slam_stage == 'auto': a.slam_stage='final' if (a.slam_output/'maps'/stages['final']).is_dir() else 'initial'
 stage_dir=a.slam_output/'maps'/stages[a.slam_stage]
 report_path=stage_dir/'report.json'
 if not report_path.is_file(): raise FileNotFoundError(f'missing {a.slam_stage} VI-BA report: {report_path}')
 ...
 report=json.loads(report_path.read_text()); intr=report['calibration']['final_intrinsics']
 ...
 (a.output/'manifest.json').write_text(json.dumps({'frames':len(out),'selected_frame_range':[a.start_index,end],'slam_stage':a.slam_stage,'pose':f'T_M_C from {a.slam_stage} VI-BA image_poses_tum.txt, exported to DA3 as T_C_M','intrinsics':f'final_intrinsics from maps/{stages[a.slam_stage]}/report.json, scaled from SLAM resolution to source video resolution','units':'meters','camera':'OpenCV RDF'},indent=2)+'\n')
