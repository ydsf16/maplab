#!/usr/bin/env bash
set -euo pipefail
repo="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"; slam=""; data=""; output=""; res=504; window_size=20; overlap=4
while [[ $# -gt 0 ]]; do case "$1" in --slam-output) slam="$2";shift 2;; --data) data="$2";shift 2;; --output) output="$2";shift 2;; --process-res) res="$2";shift 2;; --window-size) window_size="$2";shift 2;; --window-overlap) overlap="$2";shift 2;; *) echo "Unknown option: $1" >&2; exit 2;; esac; done
[[ -n "$slam" && -n "$data" && -n "$output" ]] || exit 2
(( window_size > overlap && overlap >= 0 )) || { echo "window-size must exceed window-overlap" >&2; exit 2; }
py=/root/autodl-tmp/da3/venv/bin/python; mkdir -p "$output/windows"
...
python3 "$repo/tools/sensor-recorder-geometry/export_geometry_rerun.py" --ply "$output/tsdf/tsdf_pointcloud.ply" "${camera_args[@]}" --output "$output/rerun_geometry.rrd"
python3 - <<PY
import glob, json
