...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--input',type=Path,required=True); p.add_argument('--output',type=Path,required=True); p.add_argument('--repo',type=Path,default=Path(os.environ['PHONE_AI_DA3_REPO'])); p.add_argument('--model',type=Path,default=Path(os.environ['PHONE_AI_DA3_MODEL'])); p.add_argument('--process-res',type=int,default=504); a=p.parse_args(); sys.path.insert(0,str(a.repo/'src')); from depth_anything_3.api import DepthAnything3
